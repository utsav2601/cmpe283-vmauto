...
public class VHostStatsWebService {
    ...
    public ResponseEntity getUniqueVHost() {
        try {
            logger.info("Get All Unique VHost Names");
            return new ResponseEntity<String[]>(vHostStatsService.getUniqueList(), HttpStatus.OK);
        }
        catch (Exception e) {
            logger.info("Error: " + e.getMessage());
            logger.info(e);
    ...
    public ResponseEntity getLastKnownState() {
        try {
            logger.info("Get Last known state vhost");
            return new ResponseEntity<List<VHostStats>>(vHostStatsService.getLastKnownState(), HttpStatus.OK);
        }
        catch (Exception e) {
            logger.info("Error: " + e.getMessage());
            logger.info(e);
